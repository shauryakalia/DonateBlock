...
module.exports = {
  ...
  checkOrganisationLogin: async (phone, password) => {

      let queryRes = await Organisation.find({ "phone": phone , "password": password }).lean();
      return queryRes[0];
  },
  ...
  addTokenForVerifiedOrganisation: async (db_id, jwt_token) => {

      let queryRes = await Organisation.update({_id : db_id}, { $set: { token: jwt_token}});
      return queryRes;
  },

  organisationDetails: async (db_id) => {

      let queryRes = await Organisation.findById( db_id ).lean();
      return queryRes;
  },
  ...
  addProfilePath: async (db_id, path) =>  {
    ...
    // let path  = './public/images/uploads/'+db_id;
      let queryRes  = await Organisation.update({_id: db_id}, { $set: { profile_pic: path}});
      return queryRes;
  },
