...
module.exports = {
  ...
  userDetails: async (xb_id) => {

      let queryRes = await User.findById( xb_id );
      return queryRes;
  },


  changePassword: async (xb_id, new_password) =>  {

    let queryRes = await User.update({ _id : xb_id}, { $set : { password: new_password}  });
    return queryRes;
  },


  updateUserDetails: async (xb_id, new_details) => {

      let queryRes = await User.update({_id : xb_id}, { $set : new_details});
      return queryRes;
  },

  //Add profile pic path
  addProfilePath: async (xb_id, path) =>  {

    // let path  = './public/images/uploads/'+xb_id;
      let queryRes  = await User.update({_id: xb_id}, { $set: { profile_pic: path}});
      return queryRes;
  },
